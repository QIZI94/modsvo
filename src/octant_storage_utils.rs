...
pub fn collapse_octants_from_storage<Storage, F, L>(storage: &mut Storage, start_from_id: &Storage::OctantId, mut is_leaf_predicate: F, mut make_leaf: L)  -> StorageResult<bool>
where 
	Storage: ModifiableOctantStorage,
	...
	let start_from_depth = storage.get_octant_depth(start_from_id).ok_or(StorageError::InvalidOctantId)?;
	collapse_octants_recursive(storage, start_from_depth, start_from_id, &mut is_leaf_predicate, &mut make_leaf)
}

fn collapse_octants_recursive<Storage: OctantStorage, F, L>(storage: &mut Storage, current_depth: Depth, octant_id: &Storage::OctantId, is_leaf_predicate: &mut F, make_leaf: &mut L)  -> StorageResult<bool>
where
	Storage: ModifiableOctantStorage,
	F: FnMut(Depth, &Storage::OctantId, &mut OctantStorageAccessorMut<Storage>) -> bool,
	L: FnMut(Depth, &Storage::OctantId, &mut OctantStorageAccessorMut<Storage>) -> bool	{
	...
	for maybe_child_id in children_ids.iter(){
		if let Some(child_id) = maybe_child_id{
			let mut storage_accessor = OctantStorageAccessorMut::<Storage>::new(storage);
			let is_leaf: bool = is_leaf_predicate(children_depth, &child_id, &mut storage_accessor);

			if !is_leaf {
				let child_of_child_is_leaf: bool = collapse_octants_recursive(storage, children_depth, child_id, is_leaf_predicate, make_leaf)?;
	...
	if all_children_are_leaves {
		...
		let remove_children: bool = make_leaf(current_depth, octant_id, &mut storage_accessor);
